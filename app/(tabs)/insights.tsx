...
export default function Insights(){
  ...
  const {habits,entries,palette:c}=useApp(),{elapsedDays,ranked}=buildInsightsOverview(habits,entries,todayLocal(),period),navigation=useResponsiveNavigation();
  ...
  return <ScrollView style={[s.page,{backgroundColor:c.bg}]} contentContainerStyle={s.content}>
    <View style={{minHeight:48,justifyContent:'center'}}><Text style={[s.title,{color:c.text}]}>{t('insights')}</Text></View><Text style={[s.intro,{color:c.muted}]}>{t(period==='week'?'insightsRankingWeekIntro':'insightsRankingIntro')}</Text>
    <View style={[s.periodControls,{backgroundColor:c.soft}]}>{(['week','month'] as const).map(option=><TouchableOpacity key={option} accessibilityRole="button" accessibilityState={{selected:period===option}} onPress={()=>setPeriod(option)} style={[s.periodButton,period===option&&{backgroundColor:c.card}]}><Text style={[s.periodText,{color:period===option?c.accent:c.muted}]}>{t(option)}</Text></TouchableOpacity>)}</View>
