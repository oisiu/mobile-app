...
`AppProvider` opens and migrates the database without seeding habits, exposes habit/entry snapshots, and refreshes them after writes. It centralizes daily-entry changes and transient success messages. Habit editors call repository operations and publish success after refresh.
